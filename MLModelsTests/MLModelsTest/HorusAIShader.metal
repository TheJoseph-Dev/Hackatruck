...
float3 orbit(vec2 uv, float3 baseColor, float time) {
...
float3 ripple(vec2 uv, float3 baseColor, float progress, float speed) {
    float dist = length(uv);
...
float3 circles(vec2 uv, float time) {
    float dist = length(uv);

    float s = ((sin(time)+1.0)*0.5)*0.07;
    float r1 = 0.65;
    float r2 = 0.7 + s*0.5;
    float r3 = 0.76 + s;
...
[[ stitchable ]] half4 horus(float2 pos, half4 color, float2 resolution, float time, float progress) {
    float2 uv = (pos / resolution);
    uv.y = 1 - uv.y;
    uv = uv * 2 - 1;
    uv.x *= resolution.x / resolution.y;
    
    float3 baseColor = circles(uv, time);
    baseColor = ripple(uv, baseColor, progress, 0.5);
    baseColor = orbit(uv, baseColor, time);
